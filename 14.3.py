...
def draw_board(the_board):
    ...
    while True:
        ...
        for row in range(n):        # Draw each row of the board.
            ...
            for col in range(n):    # Run through cols drawing squares
                ...
                cor_index = (color_index + 1) % 2

        # Now that squares are drawn, draw the queens.
        for (col, row) in enumerate(the_board): surface.blit(ball,
    ...
    if __name__ == "__main__":
        draw_board([0, 5, 3, 1, 6, 4, 2]) # 7 x 7 to test window size draw_board([6, 4, 2, 0, 5, 7, 1, 3]) draw_board([9,6,0,3,10,7,2,4,12,8,11,5,1]) #13x13 draw_board([11, 4, 8, 12, 2, 7, 3, 15, 0, 14, 10, 6, 13, 1, 5, 9])